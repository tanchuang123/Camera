...
{
    resize(800, 600);//主界面大小
    //resize(1000,1000);
	setMinimumSize(OneVideo::WIDTH + 20, OneVideo::HEIGHT + 50);
	createMenu();
    setStatusBar(new QStatusBar(this));//状态栏
	createContent();

	timer = new QTimer(this);
    connect(timer, SIGNAL(timeout()), SLOT(timerSlot()));//时间显示
	timer->start(1000);
}

MainWindow::~MainWindow()
{
    timer->deleteLater();//定时器
    removeWindowSlot();//删除子窗口
}
...
void MainWindow::createMenu()
{
    ...
    QAction *actionAdd = new QAction("添加窗口(&A)", menuFile);//折叠按钮
...
{
    QScrollArea *scrollArea = new QScrollArea(this);//滚动窗口
